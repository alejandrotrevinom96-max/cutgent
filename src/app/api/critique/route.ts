...
/** POST /api/critique { targetLufs?, musicClipId?, voiceClipId? } → Scorecard editorial. */
export async function POST(req: NextRequest) {
  try {
    const body = await req.json().catch(() => ({}));
    const doc = await getDocument();
    if (!doc) return NextResponse.json({ error: "No hay proyecto abierto." }, { status: 400 });
    const card = await critiqueProject(doc, {
      targetLufs: body?.targetLufs,
      musicClipId: body?.musicClipId,
      voiceClipId: body?.voiceClipId,
    });
